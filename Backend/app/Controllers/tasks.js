...
const postTask = async (req, res) => {
    ...
    if (!jsonBody) {
        res.status(400).json({"Message": "A JSON body is required for this endpoint."})
    }
    try {
...
const deleteTask = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({"Message": "Successfully deleted the following document",
...
const updateTask = async (req, res) => {
    const {objectID} = req.params;
    if (!objectID) {
        return res.status(400).json({"Message": "You must provide the ID in the path"})
    }
    try {
        const updatedDocument = await tasks.findByIdAndUpdate(objectID, req.body, {runValidators: true,
                                                                                    returnDocument: "after"})
        if (!updatedDocument) {
            return res.status(404).send("No document matched the given ObjectID")
        }
        res.status(200).json({"Message": "Successfully updated the following document.",
